    ...
    <ul class="dropdown-menu">
        @if(hasPermission('live-classes.edit'))
            <li><a class="dropdown-item edit_modal" href="javascript:void(0)"
                   data-fetch_url="{{ route('live-classes.edit',$class->id) }}"
                   data-route="{{ route('live-classes.update',$class->id) }}" data-modal="form"
                >{{__('edit') }}</a>
            </li>
        @endif
        @if(hasPermission('live-classes.destroy'))
            <li>
                <a class="dropdown-item" href="javascript:void(0)"
                   onclick="delete_row('{{ route('live-classes.destroy', $class->id) }}')"
                   data-toggle="tooltip"
