<form action="{{ route('lessons.update', $lesson->id) }}" method="POST" class="form" enctype="multipart/form-data">
    @csrf
    @method('PUT')
    <div class="row gx-20">
            ...
            <div class="mb-4">
                ...
                <div class="select-type-v2">
                    <select class="form-select form-select-lg mb-3 without_search lesson_source" name="source">
                        <option value="upload" {{ $lesson->source == 'upload' ? 'selected' : '' }}>{{ __('upload') }}</option>
                        <option value="vimeo" {{ $lesson->source == 'vimeo' ? 'selected' : '' }}>{{ __('vimeo') }}</option>
                        <option value="youtube" {{ $lesson->source == 'youtube' ? 'selected' : '' }}>{{ __('youtube') }}</option>
        ...
        <div class="col-lg-6 lesson_upload_div {{ $lesson->source != 'upload' ? 'd-none' : '' }}">
            <div class="mb-3">
                <label for="edit_lesson_{{ $lesson->id }}" class="form-label">{{ __('upload') }}</label>
                <label for="edit_lesson_{{ $lesson->id }}" class="file-upload-text">
                    <p class="file_name">{{ getFileName(getArrayValue('image', $lesson->source_data)) }}</p>
                    <span class="file-btn">{{ __('choose_file') }}</span>
                </label>
                <input class="d-none thumb_picker" name="source_data" type="file"
                    id="edit_lesson_{{ $lesson->id }}">
                <div class="nk-block-des text-danger">
                    <p class="source_data_error error">{{ $errors->first('video_file') }}</p>
                </div>
            </div>
            <a target="_blank" href="{{ static_asset(getArrayValue('image', $lesson->source_data)) }}">
                {{ __('uploaded_file') }} : {{ getFileName(getArrayValue('image', $lesson->source_data)) }}</a>
        </div>
        ...
        <div class="col-6 lesson_link {{ $lesson->source != 'upload' ? '' : 'd-none' }}">
            <div class="mb-4">
                ...
                <input type="text" class="form-control rounded-2" name="source_data"
                    value="{{ $lesson->source == 'upload' ? getFileName(getArrayValue('image', $lesson->source_data)) : $lesson->source_data }}"
                    placeholder="https://">
                <div class="nk-block-des text-danger">
        ...
        <div class="col-lg-12">
            <div class="mb-4">
                ...
                <textarea class="form-control" name="description" placeholder="{{ __('enter_description') }}">{{ $lesson->description }}</textarea>
            </div>
        </div>
