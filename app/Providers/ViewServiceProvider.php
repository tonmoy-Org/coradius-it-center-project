...
class ViewServiceProvider extends ServiceProvider
{
    ...
    public function boot()
    {
        ...
        View::composer(['frontend.layouts.base', 'frontend.layouts.master'], function ($view) {
            $meta = [
            ...
            try {
                if (request()->routeIs('instructor.details')) {
                    $slug = request()->route() ? (request()->route()->parameters()['slug'] ?? null) : null;
                    if ($slug) {
                        $user = \App\Models\User::with('instructor')->whereHas('instructor', function ($query) use ($slug) {
                ...
                // Safeguard against any unexpected errors inside view composer
            }
