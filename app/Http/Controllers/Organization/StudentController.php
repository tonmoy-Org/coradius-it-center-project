...
use App\DataTables\Organization\ActivityLogDataTable;
use App\DataTables\Organization\CheckoutDataTable;
...
class StudentController extends Controller
{
    ...
    {
        try {
            return $dataTable->render('backend.organization.student.index');
        } catch (\Exception $e) {
    ...
    public function create(CountryRepository $countryRepository): \Illuminate\Contracts\View\View|\Illuminate\Contracts\View\Factory|\Illuminate\Http\RedirectResponse|\Illuminate\Contracts\Foundation\Application
    {
        try {
            ...
            return view('backend.organization.student.create', $data);
        } catch (\Exception $e) {
            Toastr::error($e->getMessage());
    ...
    public function show($id, CourseRepository $repository)
    {
        try {
            ...
            return view('backend.organization.student.profile', $data);
        } catch (\Exception $e) {
    ...
    public function instructors($id, InstructorDataTable $dataTable)
    {
        ...
        try {
            ...
            return $dataTable->with([
                'user_id'   => $id,
                'following' => 1,
            ])->render('backend.organization.student.profile', $data);
        } catch (\Exception $e) {
            Toastr::error($e->getMessage());
    ...
    public function edit($id, CountryRepository $repository, StateRepository $stateRepository, CityRepository $cityRepository): \Illuminate\Contracts\View\View|\Illuminate\Contracts\View\Factory|\Illuminate\Http\RedirectResponse|\Illuminate\Contracts\Foundation\Application
    {
        try {
            ...
            return view('backend.organization.student.edit', $data);
        } catch (\Exception $e) {
            Toastr::error($e->getMessage());
    ...
    public function certificateDownload($id, CertificateRepository $certificateRepository)
    {
        ...
        try {
            ...
            if ($certificate) {
                $pdf      = Pdf::loadView('backend.organization.certificate.download_certificate', compact('certificate'));
                //return $pdf->stream();
