...
class StaffDataTable extends DataTable
{
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->addColumn('status', function ($user) {
                return view('backend.organization.staff.status', compact('user'));
            })->addColumn('action', function ($user) {
                return view('backend.organization.staff.action', compact('user'));
            })->addColumn('name', function ($user) {
                return view('backend.organization.staff.name', compact('user'));
    ...
    public function query(): QueryBuilder
    {
        ...
        return User::with('organizationStaff', 'lastActivity')
            ->whereHas('organizationStaff', function ($query) {
